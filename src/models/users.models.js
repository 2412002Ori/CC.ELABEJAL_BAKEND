...
export async function updateUser(id, user) {
    const { username, email, password, role_id, status, name, lastname } = user;
    const { rows } = await pool.query('UPDATE users SET username = $1, email = $2, password = $3, role_id = $4, status = $5, name = $6, lastname = $7 WHERE user_id = $8 RETURNING *',
        [username, email, password, role_id, status, name, lastname, id]
    );
    return rows[0];
}
